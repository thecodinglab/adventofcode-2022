...
shared_ptr<node> create_divider_packet(int value) {
  auto value_node = make_shared<node>();
  value_node->list = false;
  value_node->value = value;

  auto inner_list = make_shared<node>();
  inner_list->list = true;
  inner_list->children.push_back(value_node);

  auto outer_list = make_shared<node>();
  outer_list->list = true;
  outer_list->children.push_back(inner_list);
...
void run(istream &stream) {
  ...
  auto divider_a = create_divider_packet(2);
  nodes.push_back(divider_a);

  auto divider_b = create_divider_packet(6);
  nodes.push_back(divider_b);
  ...
  for (int i = 0; i < nodes.size(); i++) {
    auto node = nodes[i];
    if (node == divider_a || node == divider_b) {
      key *= (i + 1);
    }
  }
